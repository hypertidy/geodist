...
//' @param x_ Single vector of x-values in [1:n], y-values in [n+(1:n)]
//' @noRd
SEXP R_vincenty_ellips_seq (SEXP x_)
{
    size_t n = floor (length (x_) / 2);
    SEXP out = PROTECT (allocVector (REALSXP, n));
    double *rx, *rout;
    ...
    for (size_t i = 1; i < n; i++)
    {
        double L = ((rx [i] - rx [i - 1]) * M_PI / 180.0);
        double s = one_vincenty_ellips (U [i - 1], U [i], L);
        if (s < 0.0) // default to spherical vincenty
            s = one_vincenty (rx [i - 1], rx [n + i - 1],
                    rx [i], rx [n + i],
                    sin (rx [n + i - 1] * M_PI / 180.0),
                    cos (rx [n + i - 1] * M_PI / 180.0),
